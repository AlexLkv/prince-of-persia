...
ANIMATION_RIGHT = ['data\platformer/r1.png', 'data\platformer/r2.png', 'data\platformer/r3.png']
ANIMATION_LEFT = ['data\platformer/l1.png', 'data\platformer/l2.png', 'data\platformer/l3.png']

ANIMATION_JUMP_LEFT = 'data\platformer/jl.png'
ANIMATION_JUMP_RIGHT = 'data\platformer/jr.png'
ANIMATION_JUMP = 'data\platformer/j.png'
ANIMATION_STAY = 'data\platformer/0.png'
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.image = pygame.image.load(ANIMATION_STAY)
    ...
    def update(self, left, right, up, running, platforms, keys_kolvo):
        if up:
            if self.onGround:  # прыгаем, только когда можем оттолкнуться от земли
                ...
                self.image = pygame.image.load(ANIMATION_JUMP)
        ...
        if left:
            ...
            if not up:  # и не прыгаем
                ...
                if self.num_move % 4 == 0:
                    self.image = pygame.image.load(ANIMATION_LEFT[self.num_move // 4])
            if up:  # если же прыгаем
                self.image = pygame.image.load(ANIMATION_JUMP_LEFT)
            ...
            if not up:
                ...
                if self.num_move / 4 > len(ANIMATION_RIGHT) - 1:
                    self.num_move = 0
                if self.num_move % 4 == 0:
                    self.image = pygame.image.load(ANIMATION_RIGHT[self.num_move // 4])
            if up:
                self.image = pygame.image.load(ANIMATION_JUMP_RIGHT)
